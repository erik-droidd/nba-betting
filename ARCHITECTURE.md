...
- **There is ONE predict path: `nba_betting/prediction_service.py`
  (`PredictionEngine`).** Both `cli.predict` and `api/routes.py` construct it
  and call `engine.predict` — do NOT re-inline the model-load → rolling
  pipeline → feature-build → blend logic into a caller. This was previously
  duplicated, and the copies drifted: the CLI omitted split off/def Elos while
  the API passed them, so the primary path fed degraded features
  (`home_off_vs_away_def` off by ~170 Elo for a team with a wide off/def
  split — #29). The engine passes off/def Elos, the precomputed injury +
  latest-stats indices, and honors `blend` (ensemble vs raw GBM for
  `--model xgb`). Callers set `engine.injuries` / `engine.line_movements`
  before predicting and read back `engine.driver_contexts` /
  `spread_total_predictions`.
