...
def get_predictions(bankroll: float = Query(1000.0)):
    ...
    # Line movement data. Mirrors the cli.predict path: snapshots are
    # keyed by the game's ET date (`snapshot_game_date`, same as
    # Game.date / get_closing_line), with today_et() as the fallback when
    # the tipoff timestamp is missing or unparseable.
    line_movements = {}
    try:
